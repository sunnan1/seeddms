...
namespace SeedDMS\CardDAV\Backend;
...
/**
 * SeedDMS CardDAV backend
 *
 * This CardDAV backend uses SeedDMS to store addressbooks
 *
 * @copyright Copyright (C) fruux GmbH (https://fruux.com/)
 * @author Evert Pot (http://evertpot.com/)
 * @license http://sabre.io/license/ Modified BSD License
 */
class SeedDMS extends \Sabre\CardDAV\Backend\AbstractBackend implements \Sabre\CardDAV\Backend\SyncSupport {
    ...
    function getAddressBooksForUser($principalUri) {
        $addressBooks[] = [
            ...
            '{DAV:}displayname'                                           => 'Benutzerliste',
            '{' . CardDAV\Plugin::NS_CARDDAV . '}addressbook-description' => 'Alle nicht gesperrten Benutzer',
            '{http://calendarserver.org/ns/}getctag'                      => time(),
            '{http://sabredav.org/ns}sync-token'                          => 47,//time(),
        ];
        return $addressBooks;
    }
    ...
    private function __getCard($u) {
        $tmp = explode(' ', $u->getFullname(), 2);
        if(count($tmp) == 2) {
            $firstname = $tmp[0];
            $lastname = $tmp[1];
        } else {
        ...
        $vcard->add('EMAIL', $u->getEmail(), ['type'=>'work']);
        if($u->getComment())
            $vcard->add('NOTE', $u->getComment());

        /* sabre/vobject 3.3.4 doesn't set the UID by default. It must
         * be set here. Since version 4 it sets it to a self calcualted
         * value, which changes with every synchronisation of the addressbook.
         * This will add in thunderbird the complete addressbook again, when
         * it is synchronized. Just extend the VCard class and override the
         * getDefaults() method.
         */
        $vcard->add('UID', md5($u->getLogin()));
        if($u->hasImage()) {
            $img = $u->getImage();
            $mimetype = explode('/', $img['mimeType']);
            /* Need to decode the image, because VCard encodes it again. */
            $vcard->add('PHOTO', base64_decode($img['image']), ['type'=>$mimetype[1], 'encoding'=>'b']);
        }
    ...
    function getCards($addressbookId) {
        ...
        foreach($users as $u) {
            $carddata = self::__getCard($u);
            $result = array('id'=>$u->getId(), 'uri'=>trim($u->getLogin()), 'lastmodified'=>time(), 'etag'=>'"'.md5($carddata).'"', 'size'=>strlen($carddata));
            $results[] = $result;
        }
        return $results;
    ...
    function getCard($addressBookId, $cardUri) {
        ...
        if(!($u = $this->dms->getUserByLogin($cardUri)))
            return;

        $carddata = self::__getCard($u);
        $result = array('id'=>$u->getId(), 'uri'=>trim($u->getLogin()), 'lastmodified'=>time(), 'etag'=>'"'.md5($carddata).'"', 'carddata' => $carddata, 'size'=>strlen($carddata));
    ...
    function getMultipleCards($addressBookId, array $uris) {
        ...
        foreach($uris as $uri) {
            if($u = $this->dms->getUserByLogin($uri)) {
